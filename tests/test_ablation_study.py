"""Unit tests for Retrieval Ablation Study module.

Tests controlled multi-strategy ablation study runner using fake embeddings
and mocked reranking to avoid network/API calls.
"""

import json
import os
import sys
import tempfile
import unittest
from unittest.mock import patch

sys.path.insert(0, ".")

from langchain_core.embeddings import FakeEmbeddings  # noqa: E402

from src.evaluation.ablation_study import run_ablation_study  # noqa: E402


class TestAblationStudy(unittest.TestCase):
    """Test Retrieval Ablation Study runner and comparison matrix."""

    def setUp(self):
        self.fake_embeddings = FakeEmbeddings(size=64)
        self.dummy_chunks = [
            {
                "chunk_id": "doc1:chunk_1",
                "document": "doc1",
                "local_chunk_id": 1,
                "text": "Transformer self-attention mechanism.",
            },
            {
                "chunk_id": "doc1:chunk_2",
                "document": "doc1",
                "local_chunk_id": 2,
                "text": "Multi-head attention projections.",
            },
            {
                "chunk_id": "doc2:chunk_1",
                "document": "doc2",
                "local_chunk_id": 1,
                "text": "Dense passage retrieval dual encoder.",
            },
        ]

    def test_run_ablation_study(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            dataset_path = os.path.join(temp_dir, "evaluation_set.json")
            dummy_eval_data = {
                "queries": [
                    {
                        "query_id": "q001",
                        "query": "What is self-attention?",
                        "expected_relevant_chunks": [
                            {"doc_id": "doc1.pdf", "chunk_id": 1}
                        ],
                    },
                    {
                        "query_id": "q002",
                        "query": "What architecture does DPR use?",
                        "expected_relevant_chunks": [
                            {"doc_id": "doc2.pdf", "chunk_id": 1}
                        ],
                    },
                ]
            }
            with open(dataset_path, "w", encoding="utf-8") as f:
                json.dump(dummy_eval_data, f)

            mock_reranked = [
                {
                    "chunk_id": "doc1:chunk_1",
                    "chunk_text": "Transformer self-attention.",
                    "reranker_score": 0.95,
                    "metadata": {},
                }
            ]

            target_mod = "src.evaluation.ablation_study"

            with patch(
                f"{target_mod}.load_all_evaluation_chunks",
                return_value=self.dummy_chunks,
            ), patch(
                f"{target_mod}.rerank_results",
                return_value=mock_reranked,
            ):
                res = run_ablation_study(
                    data_dir=temp_dir,
                    output_dir=temp_dir,
                    k=2,
                    candidate_k=3,
                    embeddings=self.fake_embeddings,
                )

            self.assertEqual(res["num_queries"], 2)
            self.assertEqual(res["k"], 2)

            matrix = res["comparison_matrix"]
            self.assertIn("faiss_only", matrix)
            self.assertIn("bm25_only", matrix)
            self.assertIn("hybrid_rrf", matrix)
            self.assertIn("hybrid_reranked", matrix)

            output_file = os.path.join(temp_dir, "ablation_study.json")
            self.assertTrue(os.path.exists(output_file))

            with open(output_file, "r", encoding="utf-8") as f:
                saved_res = json.load(f)

            self.assertEqual(saved_res["num_queries"], 2)
            self.assertEqual(len(saved_res["comparison_matrix"]), 4)


if __name__ == "__main__":
    unittest.main()
